...
export default function ProfessorDiario({token,user}){
 const [data,setData]=useState({turmas:[]}); const [students,setStudents]=useState([]); const [records,setRecords]=useState([]); const [plans,setPlans]=useState([]); const [selectedPlan,setSelectedPlan]=useState(''); const [message,setMessage]=useState(''); const [error,setError]=useState('');
 ...
 useEffect(()=>{Promise.all([api.getProfessorDashboard(token),api.listProfessorDiaries(token),api.listProfessorLessonPlans(token)]).then(([d,r,p])=>{setData(d);setRecords(r);setPlans(p);}).catch(e=>setError(e.message));},[token]);
 ...
 function applyPlan(id){setSelectedPlan(id);const plan=plans.find(item=>String(item.id)===String(id));if(!plan)return;setForm(f=>({...f,turmaId:String(plan.turmaId),data:String(plan.data).slice(0,10),quantidadeAulas:Number(plan.quantidadeAulas||1),conteudo:plan.conteudos||'',metodologia:plan.metodologia||'',observacoes:[plan.tema?'Tema: '+plan.tema:'',plan.objetivos?'Objetivos: '+plan.objetivos:'',plan.observacoes||''].filter(Boolean).join('\n')}));setMessage('Dados do planejamento carregados. Confira a chamada e salve o diário.');}
  ...
  <main style={{maxWidth:1250,margin:'auto',padding:'28px 4%'}}>
   ...
   <form onSubmit={save} style={{display:'grid',gridTemplateColumns:'1fr 1.4fr',gap:18}}>
    <section style={box}><h2>Dados da aula</h2>
     <label style={{display:'block',marginBottom:14}}>Importar planejamento salvo<select value={selectedPlan} onChange={e=>applyPlan(e.target.value)} style={{display:'block',width:'100%',padding:11,marginTop:5}}><option value="">Preencher manualmente</option>{plans.map(p=><option key={p.id} value={p.id}>{p.tema} — {p.turma} — {new Date(p.data).toLocaleDateString('pt-BR')}</option>)}</select><small style={{display:'block',marginTop:5,color:'#607399'}}>Ao selecionar, turma, data, quantidade, conteúdo e metodologia são preenchidos automaticamente.</small></label>
