  ...
  return <div style={{ minHeight: '100vh', background: '#f4f7fc', color: '#09245a', fontFamily: 'Arial, sans-serif' }}>
    ...
    <main style={{ display: 'grid', gridTemplateColumns: '240px 1fr', minHeight: 'calc(100vh - 78px)' }}>
      <aside style={{ background: 'linear-gradient(#063675,#04285b)', color: '#fff', padding: '28px 18px' }}>
        ...
        {['Dashboard', 'Turmas', 'Professores', 'Alunos', 'Diário de classe', 'Planejamento', 'Frequência', 'Avaliações e IDEB', 'Relatórios', 'Comunicação', 'Agenda', 'Calendário escolar', 'Ocorrências'].map((item, index) => <button type="button" key={item} onClick={() => item === 'Professores' ? window.location.assign('/coordenacao/professores') : item === 'Alunos' ? window.location.assign('/coordenacao/alunos') : item === 'Calendário escolar' ? window.location.assign('/calendario-escolar') : item === 'Planejamento' ? window.location.assign('/coordenacao/planos') : setNotice(index === 0 ? 'Você já está no painel principal.' : `${item}: módulo em preparação para a próxima etapa da versão beta.`)} style={{ width: '100%', padding: '12px 10px', marginTop: index === 0 ? 18 : 2, borderRadius: 8, background: index === 0 ? '#1476ef' : 'transparent', fontWeight: 600, textAlign: 'left' }}>{item}</button>)}
