...
export default function ProfessorDashboard({ user, token, onLogout }) {
  ...
  useEffect(()=>{ Promise.all([api.getProfessorDashboard(token),api.getProfessorProfile(token),api.getProfessorCalendar(new Date().getFullYear(),token)]).then(([dashboard,personal,calendar])=>{setData(dashboard);setProfile(personal);setEvents((calendar||[]).filter(item=>new Date(`${item.data}T23:59:59`)>=new Date()).slice(0,4))}).catch(e=>setNotice(e.message)); },[token]);
  const shortcuts=[
  ...
  return <div className="siedu-legacy-portal professor-portal" style={{minHeight:'100vh',background:dark?'#101827':'#f5f8fd',color:dark?'#edf4ff':'#071f52',fontFamily:'Arial,sans-serif'}}>
    ...
    <main className="siedu-legacy-layout" style={{display:'grid',gridTemplateColumns:'250px 1fr',minHeight:'calc(100vh - 78px)'}}>
      ...
      <section className="siedu-legacy-content" style={{padding:'34px 3.5%'}}>
        ...
        <section style={{display:'grid',gridTemplateColumns:'1.15fr 1fr .8fr',gap:16,marginTop:18}}>
          ...
          <article style={panel}><h3>📅 Próximas atividades</h3>{events.length?events.map(item=><p key={`${item.origem}-${item.id}`}><b>{new Intl.DateTimeFormat('pt-BR',{day:'2-digit',month:'short'}).format(new Date(`${item.data}T12:00:00`)).toUpperCase()}</b> — {item.titulo}</p>):<p>Nenhuma atividade futura cadastrada no calendário.</p>}</article>
