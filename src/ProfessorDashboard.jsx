  ...
  return <div style={{minHeight:'100vh',background:'#f5f8fd',color:'#071f52',fontFamily:'Arial,sans-serif'}}>
    ...
    <main style={{display:'grid',gridTemplateColumns:'250px 1fr',minHeight:'calc(100vh - 78px)'}}>
      ...
      <section style={{padding:'34px 3.5%'}}>
        ...
        <h2>Acesso rápido</h2><section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:13}}>{shortcuts.map(([icon,title,desc])=><button key={title} type="button" onClick={()=>title==='Minhas Turmas'?window.location.assign('/professor/turmas'):title==='Diário de Classe'?window.location.assign('/professor/diario'):title==='Lançar Notas'?window.location.assign('/professor/notas'):title==='Planejamento de Aula'?window.location.assign('/professor/planejamento'):title==='Plano de Aula'?window.location.assign('/professor/planos'):title==='Atividades e Avaliações'?window.location.assign('/professor/atividades'):title==='Calendário Escolar'?window.location.assign('/professor/calendario'):setNotice(title+' aberto para sua área de trabalho.')} style={{...panel,border:0,textAlign:'left',display:'flex',gap:14,color:'#071f52'}}><span style={{fontSize:27}}>{icon}</span><span><b>{title}</b><small style={{display:'block',marginTop:6,color:'#566b91'}}>{desc}</small></span></button>)}</section>
