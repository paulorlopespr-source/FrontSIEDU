...
export default function AlunoPortal({ user, token, onLogout }) {
  ...
  function renderMaterials() {
    const downloadMaterial = async (item) => { try { await api.downloadClassMaterial(item.id, item.arquivoNome || item.titulo, token); } catch (downloadError) { setError(downloadError.message); } };
    return <><SectionTitle eyebrow="BIBLIOTECA DIGITAL" title="Materiais para consulta e download" description="Conteúdos publicados pelos professores da sua turma." />
      <section className="student-material-grid">{data.materiais.map((item) => <article className="student-card" key={item.id}><span className="student-file-icon">{item.tipo === 'Vídeo' ? '🎬' : item.tipo === 'Link' ? '🔗' : '📄'}</span><small>{item.tipo} · {item.disciplina}</small><h2>{item.titulo}</h2><p>{item.descricao}</p>{item.conteudoTexto && <div className="student-material-text">{item.conteudoTexto}</div>}<small>Professor(a): {item.professor}</small><div className="student-actions">{safeExternalUrl(item.urlExterna) && <a href={safeExternalUrl(item.urlExterna)} target="_blank" rel="noreferrer">Abrir conteúdo</a>}{item.possuiArquivo && <button type="button" onClick={() => downloadMaterial(item)}>⬇ Baixar {item.arquivoNome || 'arquivo'}</button>}</div></article>)}{!data.materiais.length && <Empty>Nenhum material publicado para sua turma.</Empty>}</section></>;
  }
  ...
  return <div className="student-portal">
    <aside className="student-sidebar"><div className="sidebar-prefeitura-logo"><img src="/images/prefeitura.png" alt="Prefeitura Municipal de Pindobaçu"/></div><Link className="sidebar-system-home" to="/aluno" aria-label="Voltar ao início do Portal do Aluno"><img src="/images/siedu-logo-transparent.svg" alt="SIEDU — Sistema Integrado de Educação"/></Link><div className="student-brand"><div><strong>SIEDU</strong><small>Portal do Aluno</small></div></div><nav>{sections.map(([slug, icon, label]) => <Link className={selected === slug ? 'active' : ''} to={slug === 'inicio' ? '/aluno' : `/aluno/${slug}`} key={slug}><span>{icon}</span>{label}{slug === 'notificacoes' && data.resumo.notificacoesNaoLidas > 0 && <i>{data.resumo.notificacoesNaoLidas}</i>}</Link>)}</nav><div className="student-sidebar-footer"><b>Prefeitura de Pindobaçu</b><small>Secretaria Municipal de Educação</small></div></aside>
