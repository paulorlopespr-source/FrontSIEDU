 ...
 return <div style={{minHeight:'100vh',background:'#f4f7fc',color:'#09245a',fontFamily:'Arial'}}>
  ...
  <section style={{display:'grid',gridTemplateColumns:'minmax(330px,.8fr) minmax(500px,1.5fr)',gap:20}}>
   ...
   <section><div style={{...card,marginBottom:16,display:'flex',alignItems:'center',gap:14}}><h2 style={{margin:0}}>📂 Biblioteca da aula</h2><select value={filter} onChange={e=>setFilter(e.target.value)} style={{...input,width:'auto',marginLeft:'auto'}}><option value="">Todas as turmas</option>{data.turmas.map(t=><option key={t.id} value={t.id}>{t.nome}</option>)}</select></div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:14}}>{items.map(item=><article key={item.id} style={card}><div style={{fontSize:36}}>{icons[item.tipo]||'📎'}</div><small>{item.tipo} · {item.disciplina}</small><h3>{item.titulo}</h3><p><b>{item.turma}</b></p>{item.descricao&&<p>{item.descricao}</p>}{item.conteudoTexto&&<div style={{whiteSpace:'pre-wrap',background:'#f4f7fc',padding:10,borderRadius:7,maxHeight:150,overflow:'auto'}}>{item.conteudoTexto}</div>}<div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}>{item.urlExterna&&<a href={item.urlExterna} target="_blank" rel="noreferrer">🔗 Abrir link</a>}{item.possuiArquivo&&<button type="button" onClick={()=>download(item)}>⬇️ Baixar {item.arquivoNome||'arquivo'}</button>}</div><div style={{display:'flex',gap:8,marginTop:14,borderTop:'1px solid #edf1f7',paddingTop:10}}><button type="button" onClick={()=>edit(item)}>✏️ Alterar</button><button type="button" onClick={()=>remove(item)} style={{color:'#a52222'}}>🗑️ Excluir</button></div><small style={{display:'block',marginTop:10,color:'#607399'}}>Professor: {item.professor}</small></article>)}{!items.length&&<div style={card}>Nenhum material publicado.</div>}</div>
   </section>
  </section></main>
 </div>
}
